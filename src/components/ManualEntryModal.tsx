...
export default function ManualEntryModal({ isOpen, onClose, userId, selectedDate, existingLog }: ManualEntryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-end sm:items-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">

      <div className="bg-white rounded-t-xl sm:rounded-xl shadow-xl w-full h-full sm:h-auto sm:max-w-md overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-4 sm:space-y-5 flex-grow overflow-y-auto">
          {error && (
          ...
          {/* Modal Footer (FIXED) */}
          <div className="pt-4 mt-2 sm:mt-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0 shrink-0 border-t border-gray-100">
            {existingLog ? (
              <button
                type="button"
                onClick={handleDelete}
                disabled={isSubmitting}
                className="w-full sm:w-auto flex items-center justify-center gap-1 text-red-600 hover:bg-red-50 px-3 py-3 sm:py-2 rounded-lg text-sm font-medium transition-colors touch-manipulation order-2 sm:order-1 border border-transparent hover:border-red-100"
              >
            ...
            <div className="flex flex-col sm:flex-row gap-3 order-1 sm:order-2 w-full sm:w-auto">
              <button
                ...
                onClick={onClose}
                disabled={isSubmitting}
                className="w-full sm:w-auto px-4 py-3 sm:py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 transition-colors touch-manipulation order-2 sm:order-1"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full sm:w-auto px-4 py-3 sm:py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors touch-manipulation order-1 sm:order-2"
              >
